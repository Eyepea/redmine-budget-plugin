...
class Deliverable < ActiveRecord::Base
  ...
  # Collect time_entries
  def collect_time_entries
    return self.issues.collect(&:time_entries).flatten.delete_if{ |te| te.activity_id == Setting.plugin_budget_plugin['budget_non_billable_activity'].to_i}
  end

  # Number of hours used.
  def hours_used
    return 0 unless self.issues.size > 0
    return collect_time_entries.collect(&:hours).sum
  end
